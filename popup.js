document.addEventListener('DOMContentLoaded', () =>
{
    ...
    function populatePermissions(permissionsFromContent)
    {
        ...
        if (permissions.length===0&&permissionsFromContent===null)
        {
            ...
            { permissionsUnavailableMsgEl.textContent='Permission data could not be loaded.'; permissionsUnavailableMsgEl.style.display='block';}
            return { granted:[], prompt:[] };
        }
    ...
    function calculateAndDisplayPrivacyScore(data, contentError)
    {
        ...
        (data.permissionsGranted||[]).forEach(perm =>
        {
            ...
            { penalty=15; remarks.push(`${perm} GRANTED!`); }
            else if (perm==='Location')
            { penalty=10; remarks.push("Location GRANTED."); }
            if (penalty>0)
            {
        ...
        (data.permissionsPrompt||[]).forEach(perm =>
        {
            let penalty=0;
            if (perm==='Camera'||perm==='Microphone')
            { penalty=7; remarks.push(`May request ${perm}.`); }
            else if (perm==='Location')
            { penalty=5; remarks.push("May request Location."); }
            if (penalty>0)
            {
                 currentScore-=penalty;
        ...
        if (remarks.length===0&¤tScore===100&&!contentError) remarkText="Looking good! Excellent privacy practices detected.";
        ...
        if (currentScore<50)
        { scoreColor="#F44336"; if ((remarks.length==0||remarks.every(r=>r.includes("Partial data analysis.")))&&!contentError ) remarkText="Caution: Low score.";}
        else if (currentScore<75)
        { scoreColor="#FFC107"; if ((remarks.length==0||remarks.every(r=>r.includes("Partial data analysis.")))&&!contentError ) remarkText="Review for concerns.";}
    ...
    function checkEmptySections()
    {
        querySelAll('.dt-section').forEach(section =>
        {
            ...
            if (generalEmptyMsgForSection&§ion.id!=='permissionsSection'&§ion.id!=='connectionsSection')
            {
