...
document.addEventListener('DOMContentLoaded',()=>
{
    ...
    {
        const listEl=getElem('cookieList');
        const countEl=getElem('cookieCount');
        if(!listEl||!countEl)return 0;
        listEl.innerHTML='';
    ...
    function populateConnections(domains,pageTopLevelDomain,currentTabExactHostname)
    {
        ...
        connListTrackersEl.innerHTML='';
        connListOtherExternalEl.innerHTML='';
        ...
        let trackersListContent=[];
        let otherExternalListContent=[];
    ...
    {
        const lsListEl=getElem('localStorageList');
        const ssListEl=getElem('sessionStorageList');
        const countEl=getElem('storageCount');
        const emptyMsg=getElem('storageEmptyMessage');
        if(!lsListEl||!ssListEl||!countEl)return 0;
        lsListEl.innerHTML='';
        ssListEl.innerHTML='';
        let totalItems=0;
        const lsData=(pageData&&pageData.localStorage)||[];
        const ssData=(pageData&&pageData.sessionStorage)||[];
        const lsCount=(pageData&&pageData.localStorageCount)||0;
        const ssCount=(pageData&&pageData.sessionStorageCount)||0;
        ...
        if(emptyMsg)emptyMsg.style.display=totalItems===0&&lsListEl.children.length===0&&ssListEl.children.length===0?'block':'none';
         ...
         {
            emptyMsg.textContent=`Found ${totalItems} storage items (details limited).`;
    ...
    function populatePermissions(permissionsFromContent)
    {
        ...
        permListAllowedEl.innerHTML='';
        permListAskEl.innerHTML='';
        permListBlockedEl.innerHTML='';
        ...
        if(permissions.length===0&&permissionsFromContent===null)
        {
            ...
            if(permissionsUnavailableMsgEl)
            {
                permissionsUnavailableMsgEl.textContent='Permission data could not be loaded.';
        ...
        permissions.forEach(p=>
        {
            ...
            else
            {
                ...
                note.textContent=`(${p.status})`;
                const detailSpan=listItem.querySelector('.dt-item-detail');
                if(detailSpan)detailSpan.appendChild(note);
                else listItem.appendChild(note);
                permListAskEl.appendChild(listItem);
                askCount++;
            }
        });
    ...
    function calculateAndDisplayPrivacyScore(data,contentError)
    {
        ...
        (data.permissionsGranted||[]).forEach(perm=>
        {
            ...
            {
                penalty=15;
                remarks.push(`${perm} GRANTED!`);
            }
            else if(perm==='Location')
            {
                penalty=10;
                remarks.push("Location GRANTED.");
            }
            if(penalty>0)
            {
        ...
        (data.permissionsPrompt||[]).forEach(perm=>
        {
            ...
            {
                penalty=7;
                remarks.push(`May request ${perm}.`);
            }
            else if(perm==='Location')
            {
                penalty=5;
                remarks.push("May request Location.");
            }
            if(penalty>0)
            {
                 currentScore-=penalty;
        ...
        {
            scoreColor="#F44336";
            if((remarks.length==0||remarks.every(r=>r.includes("Partial data analysis.")))&&!contentError)remarkText="Caution: Low score.";
        }
        else if(currentScore<75)
        {
            scoreColor="#FFC107";
            if((remarks.length==0||remarks.every(r=>r.includes("Partial data analysis.")))&&!contentError)remarkText="Review for concerns.";
        }
        updatePrivacyScore(currentScore,remarkText,scoreColor,adviceItems,totalPossibleIncrease);
    }
                ...
                else
                {
                    ...
                    else
                    {
                        isEffectivelyEmpty=!Array.from(content.children).some(child=>
                            ...
                            !child.classList.contains('dt-empty-message')&&
                            !child.classList.contains('dt-error-message')&&
                ...
                try
                {
                    ...
                    else if(pageDataResult.status==='rejected')
                    {
                        ...
                        console.error("DigitalTrustExt_Popup_Corrected_v2: PageData fetch error (sendMessage FAILED):",pageDataResult.reason);
                         ...
                         else if(permissionsUnavailableMsgEl)
                         {
