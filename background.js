// background.js - Digital Trust Extension
    ...
    if (details.tabId > 0 && details.url) {
      ...
      try {
        ...
        if (details.type === "main_frame") {
             ...
             if(tabRequests[details.tabId]) tabRequests[details.tabId].clear(); // Clear on main_frame navigation
             tabRequests[details.tabId].add(requestHostname);
        }
      } catch (e) { /* Silently ignore parse errors for robustness */ }
    }
  },
  { urls: ["<all_urls>"] }
);
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.url && tab && tab.url) {
    ...
    try {
        const newUrl = new URL(changeInfo.url); // URL that changed
        const newMainHostname = newUrl.hostname.replace(/^www\./, '');

        // This logic should ideally align with main_frame navigation,
        // onBeforeRequest is often more precise for clearing data.
        // This can serve as a fallback if main_frame was missed.
